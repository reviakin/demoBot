...
export const botStart = async () => {
  try {
    ...
    bot.on('new_chat_members', nMember => {
      bot.sendMessage(
        nMember.chat.id,
        `Приветствуем в нашем чате. Будьте вежливы и с утверждением обращаться к конкретному пользователю. Расскажите как вас зовут, что вы умеете и чему хотите научиться.`
      )
    })
  } catch (e) {
    console.error(e)
